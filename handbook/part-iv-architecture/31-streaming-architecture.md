...
The current implementation covers response start, text delta, continuation metadata, tool-call deltas, finish reason, and response end for OpenAI-compatible and Anthropic-compatible SSE. Anthropic input JSON fragments remain opaque until the generation loop has a complete tool call. The byte framer tolerates split chunks, CRLF boundaries, multiline data, comments, and a final event without a trailing blank line.
