...
enum AgentInstructions {
    ...
    Web artefacts may be full source-plus-preview workspaces. inlineWeb artefacts are compact, self-contained, responsive HTML/CSS/JavaScript previews intended to appear inline inside a conversation. Ginny injects a shadcn-compatible design token sheet and a Tailwind-style utility subset into every web preview. Use the provided CSS variables and utilities such as bg-background, text-foreground, bg-primary, text-primary-foreground, border, rounded-lg, flex, grid, gap-4, p-4, and text-muted-foreground instead of redefining the palette. Keep previews local-only and safe to embed. When making a web or inlineWeb artefact, call discover_skills and then read_skill for the relevant frontend design skill before writing the artefact.
